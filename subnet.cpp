...
Subnet::Subnet(string s) {
	...
	if(2 == sscanf(s.c_str(), "%[0-9.]/%hhu", p, &m)) {
		...
		if(m > 32) {
			start = end = offset = 0;
            cerr << "Bad IPv4 subnet mask: " << s << endl;
			return;
		}
	...
	} else if(2 == sscanf(s.c_str(), "%[a-fA-F0-9:]/%hhu", p, &m)) {
        cout << "Got IPv6 prefix: " << p << " mask: " << int(m) << endl;
		if(m > 128) {
			start = end = offset = 0;
            cerr << "Bad IPv6 subnet mask: " << s << endl;
			return;
		}
	} else {
        cerr << "Error parsing subnet string: " << s << endl;
	}
...
// ROB we only want 2^5=32 different TTL values, so lob off 3 bits
