...
void *Patricia::get(const char *string, bool exact) {
...
int Patricia::parseBGPLine(char *_line, std::string *net, uint32_t *asn) {
    ...
    std::string::size_type first_space = 0, last_space = 0;
    if (line.find('>') == std::string::npos) {
        return 0;
    }
    first_space = line.find(' ');
    last_space = line.rfind(' ');
    *net = line.substr(1,first_space);
...
void Patricia::populate(const char *filename) {
